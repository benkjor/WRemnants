    ...
    for i, h_pred in enumerate(hists_pred):
        ...
        with open(logname, "a") as logf:
            ...
            for i in range(len(hists_proc)):
                v = str(round(np.average(hists_proc[i].values())))
...
def main():
    ...
    low_data = debug.data_obs_hists["ch_iso_poi_low"].project("mll")
    high_data = debug.data_obs_hists["ch_iso_poi_high"].project("mll")
    mll_ax = hist.axis.Variable(
        np.concatenate((low_data.axes.edges[0], high_data.axes.edges[0])),
        name="mll",
        overflow=False,
        underflow=False,
    )
    center_data = np.array([debug.nominal_hists["ch_iso_eff"].sum()])

    hist_data = hist.Hist(mll_ax)
    hist_data_vals = np.concatenate(
        (low_data.values(), center_data, high_data.values()), axis=0
    )
    hist_data.values()[...] = hist_data_vals

    low_proc = debug.nominal_hists["ch_iso_poi_low"].project("mll", "processes")
    high_proc = debug.nominal_hists["ch_iso_poi_high"].project("mll", "processes")
    center_proc = debug.nominal_hists["ch_iso_eff"].project(
        "processes"
    )  ## these are normalized later
    proc_ax = high_proc.axes[1]
    hist_proc = hist.Hist(mll_ax, proc_ax)
    hist_proc_vals = np.concatenate(
        (low_proc.values(), center_proc.values()[np.newaxis, ...], high_proc.values()),
        axis=0,
    )
    hist_proc.values()[...] = hist_proc_vals
